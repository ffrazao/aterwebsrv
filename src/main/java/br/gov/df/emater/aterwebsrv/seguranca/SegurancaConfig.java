...
public class SegurancaConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		String[] permitAll = { "/", "/exemplo/**", "/dominio/**", "/enquete-anonima/**", "/enumeracao/**", "/erro", "/login", "/logout", "/mudarSenhaAtual", "/pesquisa-textual/**", "/resources/**", "/testDB.jsp", "/tiles/enquete-resp/**", "/tiles/exibe-erro/**" };
...
//		http.formLogin().loginPage("/login").defaultSuccessUrl("/").failureUrl("/login?login_error=true");
	...
	public SegurancaAutenticador setupCustomAuthenticationProvider() {
		// Customiza��o da autentica��o do usu�rio
		SegurancaAutenticador result = new SegurancaAutenticador();
		return result;
	}

	@Bean(name = "customUsernamePasswordAuthenticationFilter")
	public SegurancaAutenticadorFiltro setupCustomUsernamePasswordAuthenticationFilter() throws Exception {
		// Necessario para habilitar a capta��o do campo administrador da tela
		// de login
		SegurancaAutenticadorFiltro result = new SegurancaAutenticadorFiltro();
